...
def accuracy(y_hat, y):
    """Compute element-wise binary accuracy between predictions and ground truth."""
    assert y_hat.shape == y.shape, "y_hat and y are not the same! {} {}".format(y_hat.shape, y.shape)
    y_hat = y_hat.cpu().detach()
    y = y.cpu()
    mask = (y_hat > 0.5).float()  # noqa: PLR2004
...
class BinaryClassMetrics:
    ...
    def compute_fscore(self):
        ...
        return 2.0 * (precision * recall / sumDiv) if sumDiv > 0.0 else 0.0
    ...
    def compute_area_under_curve(self, pNumSteps, curve):
        assert curve in ["ROC", "PR"], (
            "Please provide a curve from: Receiver Characteristic Operator (ROC) or Precision Recall (PR)"
        )
        if curve == "PR":
