...
def conv_bn_relu(in_channels, out_channels, downsampling):
    """Conv2d (strided) + BatchNorm2d + ReLU."""
    return nn.Sequential(
        conv3x3(in_channels, out_channels, stride=downsampling),
...
def bn_act_drop(num_channels, dropout):
    """BatchNorm1d + ReLU + optional Dropout."""
    if dropout > 0:
...
class Decoder(nn.Module):
    def __init__(self, latent_size=128, dropout=0.5, n_features=None, apply_sigmoid=False):
